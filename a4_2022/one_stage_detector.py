...
class FCOSPredictionNetwork(nn.Module):
    ...
    def __init__(
        ...
        # box. The prediction layers for box regression and centerness
        ...
        for idx in range(len(stem_channels)-1):
            stem_cls.append(nn.Conv2d(stem_channels[idx], stem_channels[idx+1], kernel_size=3, stride=1, padding=1))
            stem_cls.append(nn.ReLU())

            stem_box.append(nn.Conv2d(stem_channels[idx], stem_channels[idx+1], kernel_size=3, stride=1, padding=1))
    ...
    def forward(self, feats_per_fpn_level: TensorDict) -> List[TensorDict]:
        ...
        for level_name, level_tensor in feats_per_fpn_level.items():
            class_tensor = self.pred_cls(self.stem_cls(level_tensor))
            boxreg_tensor = self.pred_box(self.stem_box(level_tensor))
            centerness_tensor = self.pred_ctr(self.stem_box(level_tensor))

            class_tensor = class_tensor.flatten(start_dim=-2, end_dim=-1).permute(0, 2, 1)
            boxreg_tensor = boxreg_tensor.flatten(start_dim=-2, end_dim=-1).permute(0, 2, 1)
            centerness_tensor = centerness_tensor.flatten(start_dim=-2, end_dim=-1).permute(0, 2, 1)
            
            class_logits[level_name] = class_tensor
            boxreg_deltas[level_name] = boxreg_tensor
            centerness_logits[level_name] = centerness_tensor
...
def fcos_get_deltas_from_locations(
    ...
    ##########################################################################
    # Deltas is a Tensor of shape (N, 4) giving deltas (left, top, right, bottom)
    # from the locations to GT box edges, normalized by FPN stride.

    xc, yc = locations[:, 0], locations[:, 1]

    L = ((xc - gt_boxes[:, 0]) / stride).unsqueeze(dim=1)
    T = ((yc - gt_boxes[:, 1]) / stride).unsqueeze(dim=1)
    R = ((gt_boxes[:, 2] - xc) / stride).unsqueeze(dim=1)
    B = ((gt_boxes[:, 3] - yc) / stride).unsqueeze(dim=1)

    deltas = torch.cat((L, T, R, B), dim=1)
    gt_boxes_without_class = gt_boxes[:, 0:4]
    deltas[gt_boxes_without_class[:, 0] == -1] = -1  # place -1 in any 2D location where the input was -1
...
def fcos_apply_deltas_to_locations(
    ...
    deltas = deltas.clamp(min=0)  # if the input deltas are -1, the outputs are the input locations

    output_boxes[:, 0] = xc - deltas[:, 0] * stride
    output_boxes[:, 1] = yc - deltas[:, 1] * stride
    output_boxes[:, 2] = xc + deltas[:, 2] * stride
    output_boxes[:, 3] = yc + deltas[:, 3] * stride
    ##########################################################################
    #                             END OF YOUR CODE                           #
    ##########################################################################
...
class FCOS(nn.Module):
    ...
    def forward(
        ...
        # boxes for locations per FPN level, per image in the batch. Fill this list:
        matched_gt_boxes = []
        ...
        for i in range(B):
            matched_gt_deltas_per_fpn_level = {
                'p3': fcos_get_deltas_from_locations(locations_per_fpn_level['p3'], matched_gt_boxes[i]['p3'], stride=8),
                'p4': fcos_get_deltas_from_locations(locations_per_fpn_level['p4'], matched_gt_boxes[i]['p4'], stride=16),
                'p5': fcos_get_deltas_from_locations(locations_per_fpn_level['p5'], matched_gt_boxes[i]['p5'], stride=32)
            }
        ...
        # (locations_per_fpn_level has length H*W, H and W referring to each level)
        matched_gt_boxes = default_collate(matched_gt_boxes)  # contains class dim
        matched_gt_deltas = default_collate(matched_gt_deltas)  # not contains class dim
        ...
        bg_mask = gt_classes < 0  # background classes
        gt_classes_one_hot = torch.nn.functional.one_hot(gt_classes.clamp(min=0).to(torch.int64), self.num_classes)
        ...
        loss_box = 0.25 * torch.nn.functional.l1_loss(input=pred_boxreg_deltas, target=matched_gt_deltas, reduction='none')
        loss_box[bg_mask] *= 0

        # fcos_make_centerness_targets() works on [_,4] tensor, but matched_gt_deltas has shape [_,_,4].
        # It can be viewed as a [-1,4] tensor. The output of the method has shape [_,1].
        # pred_ctr_logits has shape [_,_,1]. It is viewed as well as a (-1,1) tensor.
        # Since both tensors are views of shape [-1,1], positions match and loss can be computed.
        matched_gt_ctr = fcos_make_centerness_targets(matched_gt_deltas.view(-1, 4)).unsqueeze(dim=1)
        loss_ctr = torch.nn.functional.binary_cross_entropy_with_logits(input=pred_ctr_logits.view(-1,1), target=matched_gt_ctr, reduction="none")
    ...
    def inference(
        ...
        for level_name in locations_per_fpn_level.keys():
            ...
            # pred_cls_prob = level_pred_classes.sigmoid()
            
            # Compute geometric mean of class logits and centerness:
            level_pred_all_scores = torch.sqrt(
                level_cls_logits.sigmoid() * level_ctr_logits.sigmoid()
            )
            # Step 1:
            # For each location, keep class with highest score
            level_pred_scores, level_pred_classes = level_pred_all_scores.max(dim=1)
            ...
            level_pred_boxes = level_pred_boxes[retain_mask]

            # remove background 
            notBackgroundMask = (level_deltas[retain_mask].sum(dim=1) != -4)
            level_pred_scores = level_pred_scores[notBackgroundMask]
            level_pred_classes = level_pred_classes[notBackgroundMask]
            level_pred_boxes = level_pred_boxes[notBackgroundMask]
