...
class VAE(nn.Module):
    def __init__(self, input_size, latent_size=15, hidden_dim=400):
        ...
        ###########################################################################
        # Hidden dim 400 as defined in 498_FA2019_lecture20.pdf, slide 12/120.
...
class CVAE(nn.Module):
    def __init__(self, input_size, num_classes=10, latent_size=15, hidden_dim=400):
    ...
    def forward(self, x, c):
        ...
        #     resconstruct x                                                      #
        ###########################################################################
        enc_in = torch.cat((torch.flatten(x, start_dim=1), c), dim=1)
...
def reparametrize(mu, logvar):
    ...
    z = mu + epsilon * torch.sqrt(torch.exp(logvar))
    ###############################################################################
    #                              END OF YOUR CODE                               #
    ###############################################################################
    return z
...
def loss_function(x_hat, x, mu, logvar):
    ...
    kldiv_loss = -0.5 * ((1 + logvar).sum() - (mu**2).sum() - torch.exp(logvar).sum())

    loss = (reconstruction_loss + kldiv_loss) / N
    ###############################################################################
    #                            END OF YOUR CODE                                 #
    ###############################################################################
    return loss
