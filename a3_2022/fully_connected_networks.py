...
class TwoLayerNet(object):
    ...
    def loss(self, X, y=None):
        ...
        out1, fc1_cache = Linear.forward(X, W1, b1)
        out2, relu_cache = ReLU.forward(out1)
        scores, fc2_cache = Linear.forward(out2, W2, b2)
        ##############################################################
        #                     END OF YOUR CODE                       #
        ##############################################################
        ...
        dout2, dW2, db2 = Linear.backward(dscores, fc2_cache)
        dout1 = ReLU.backward(dout2, relu_cache)
        dX, dW1, db1 = Linear.backward(dout1, fc1_cache)
...
class FullyConnectedNet(object):
    ...
    def __init__(self, hidden_dims, input_dim=3*32*32, num_classes=10,
        ...
        self.reg = reg
        self.num_layers = 1 + len(hidden_dims) # including last (output) layer
        self.dtype = dtype
        self.params = {}
        ...
        for layer in range(self.num_layers):
          layer_number_name = layer + 1
          layer_in = all_dims[layer]
          layer_out = all_dims[layer + 1]
          self.params[f'W{layer_number_name}'] = torch.normal(0, weight_scale, (layer_in, layer_out), dtype=dtype, device=device)
          self.params[f'b{layer_number_name}'] = torch.zeros(layer_out, dtype=dtype, device=device)
        #######################################################################
        #                         END OF YOUR CODE                            #
        #######################################################################
    ...
    def loss(self, X, y=None):
        ...
        # Until the last layer, Linear + ReLU (+ Dropout)
        ...
        loss_data, dout = softmax_loss(h_out, y)

        for i in range(self.num_layers):
          w = self.params['W' + str(i+1)]
          loss_w += self.reg * torch.sum(w ** 2)

        loss = loss_data + loss_w
        
        # Last layer is only Linear forward
        ...
        for i in range(0, self.num_layers-1):
          ...
          w = self.params[f'W{self.num_layers -1 - i}']
          grads[f'W{self.num_layers -1 - i}'] = dw + 2 * self.reg * w
          grads[f'b{self.num_layers -1 - i}'] = db
        ############################################################################
        #                             END OF YOUR CODE                             #
        ############################################################################
...
def create_solver_instance(data_dict, dtype, device):
    ...
    #############################################################
    optim_config={'learning_rate': 1e-1, 'lr_decay': 0.9}
...
def get_three_layer_network_params():
    ...
    learning_rate = 1e0  # Experiment with this!
    ###############################################################
...
class Dropout(object):
    ...
    def forward(x, dropout_param):
        ...
        if mode == 'train':
            ...
            ##############################################################
            mask = torch.rand(x.shape, dtype=x.dtype, device=x.device) > p
            out = (x * mask) / (1 - p)
            ##############################################################
            #                   END OF YOUR CODE                         #
            ##############################################################
    ...
    def backward(dout, cache):
        ...
        if mode == 'train':
            ...
            ###########################################################
            dx = (dout * mask) / (1 - p)
            # By the chain rule, same constant applies here.
            # From another perspective, the derivatives have to be rescaled
            # to keep the statistics
            ###########################################################
            #                     END OF YOUR CODE                    #
            ###########################################################
        elif mode == 'test':
